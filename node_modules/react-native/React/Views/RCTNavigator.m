...
BOOL jsGettingtooSlow =
  ...
  if (jsGettingAhead) {
    if (reactPushOne) {
      ...
      [_navigationController pushViewController:vc animated:!vc.navItem.animationSubtype?(currentReactCount > 1):NO];

      // 动画效果
      if (vc.navItem.animationSubtype) {
        isAnimation = YES;
        NSLog(@"=============%@",vc.navItem.animationSubtype);
        
        CATransition *tran=[CATransition animation];
        tran.duration=0.5f;
        tran.type=@"moveIn";//moveIn(新视图移到旧视图上面);rippleEffect(滴水效果(不支持过渡方向))
        tran.subtype=vc.navItem.animationSubtype;
      ...
      if (isAnimation) {
        ...
        CATransition *tran=[CATransition animation];
        tran.duration=0.5f;
        tran.type=@"reveal";//moveIn(新视图移到旧视图上面);rippleEffect(滴水效果(不支持过渡方向))
        tran.subtype=@"fromBottom";
        [self.navigationController.view.layer addAnimation:tran forKey:nil];
      }
      
      [_navigationController popToViewController:viewControllerToPopTo animated:isAnimation];
