...
BOOL jsGettingtooSlow =
  ...
  if (jsGettingAhead) {
    if (reactPushOne) {
      ...
      [_navigationController pushViewController:vc animated:!_animationParam?(currentReactCount > 1):NO];

//      NSLog(@"====> %@", _animationParam);
      // 动画效果
      if (_animationParam) {
        // 获取视图上传递进来的动画参数['moveIn', 'fromTop', 'reveal', 'fromBottom']，
        // 前2为视图切入的动画方式和方向，后2为视图离开的动画方式和方向
        CATransition *tran=[CATransition animation];
        tran.duration=0.5f;
        tran.type=_animationParam[0];//moveIn(新视图移到旧视图上面);rippleEffect(滴水效果(不支持过渡方向))
        tran.subtype=_animationParam[1];
...
//      NSLog(@"====> %@", _animationParam);
      ...
      if (_animationParam) {
        ...
        CATransition *tran=[CATransition animation];
        tran.duration=0.5f;
        tran.type=_animationParam[0];
        tran.subtype=_animationParam[1];
        [self.navigationController.view.layer addAnimation:tran forKey:nil];
      }
      
      [_navigationController popToViewController:viewControllerToPopTo animated:_animationParam?NO:YES];
